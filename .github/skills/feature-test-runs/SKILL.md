...
`get_scenario_results` to run scenarios and check results; `get_scenario_errors`
then `get_scenario_logs` to see why one failed. Each
...
(`create_campaign`, `control_campaign`, `await_campaign_events`). They keep the
